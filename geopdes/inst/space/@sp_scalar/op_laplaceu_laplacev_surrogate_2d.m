% OP_LAPLACEU_LAPLACEV_SURROGATE_2D: assemble the matrix A = [a(i,j)], a(i,j) = (epsilon laplace u_j, laplace v_i),
% exploiting the tensor product structure using the surrogate method.
%
%   mat = op_laplaceu_laplacev_surrogate_2d (space, msh, epsilon, M, q);
%   [rows, cols, values] = op_laplaceu_laplacev_surrogate_2d (space, msh, epsilon, M, q);
%
% INPUT:
%
...
function varargout = op_laplaceu_laplacev_surrogate_2d(space, msh, coeff, M, q)

if msh.ndim ~= 2
  error('op_laplaceu_laplacev_surrogate_2d: This function only supports 2D');
end

if ~all(space.ndof_dir == space.ndof_dir(1))
  error('op_laplaceu_laplacev_surrogate_2d: Dofs in each dimension must be equal');
end
...
elseif q == 3
  ...
  error('op_laplaceu_laplacev_surrogate_2d: q = %d is not supported', q);
end
...
for iel = 1:msh.nel_dir(1)
  if ismember(iel, boundary_mask{1}) % Case corresponding to the red elements in the article
    ...
    sp_col = sp_evaluate_col(space, msh_col, 'value', false, 'gradient', false, 'laplacian', true);
  elseif ismember(iel, element_mask{1}) % Case corresponding to the blue elements in the article
    ...
    sp_col = sp_evaluate_col(space, msh_col, 'value', false, 'gradient', false, 'laplacian', true, 'element_mask', element_mask);
  else % Case corresponding to the green elements in the article
    msh_col = msh_evaluate_col(msh, iel, 'element_mask', boundary_mask);
    sp_col = sp_evaluate_col(space, msh_col, 'value', false, 'gradient', false, 'laplacian', true, 'element_mask', boundary_mask);
  end
  ...
  K_surr = K_surr + op_laplaceu_laplacev(sp_col, sp_col, msh_col, coeffs);
end
