...
function varargout = op_general_su_ev_surrogate_2d (space, msh, C, surrogate_opts)
  ...
  if msh.ndim ~= 2
    error('op_general_su_ev_surrogate_2d: This function only supports 2D');
  end

  if ~all(space.ndof_dir == space.ndof_dir(1))
    error('op_general_su_ev_surrogate_2d: Dofs in each dimension must be equal');
  end
  ...
  elseif surrogate_opts.degree == 3
    ...
    error('op_general_su_ev_surrogate_2d: q = %d is not supported', q);
  end
