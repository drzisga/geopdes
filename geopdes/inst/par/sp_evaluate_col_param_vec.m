...
function sp = sp_evaluate_col_param_vec (space, msh, varargin)
...
for icomp = 1:space.ncomp_param
  if element_mask_enabled
    sp_col_scalar(icomp) = sp_evaluate_col_param_scal (space.scalar_spaces{icomp}, msh, 'value', value, 'gradient', first_der, 'hessian', hessian, 'element_mask', element_mask);
  else
    sp_col_scalar(icomp) = sp_evaluate_col_param_scal (space.scalar_spaces{icomp}, msh, 'value', value, 'gradient', first_der, 'hessian', hessian);
  end
end
