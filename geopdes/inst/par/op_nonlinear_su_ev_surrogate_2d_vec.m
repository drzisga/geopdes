...
function varargout = op_nonlinear_su_ev_surrogate_2d_vec (space, msh, Stress, DStress, u_old, surrogate_opts)
  ...
  element_mask{1} = sp_get_cells_vec(space, msh, row_indices_subset.');
  ...
  for iel = 1:msh.nel_dir(1)
    ...
    elseif ismember(iel, element_mask{1}) % Case corresponding to the blue elements in the article
      msh_col = msh_evaluate_col_cart(msh, iel, 'element_mask', element_mask);
      sp_col = sp_evaluate_col_vec(space, msh_col, 'value', false, 'gradient', true, 'element_mask', element_mask);
    else % Case corresponding to the green elements in the article
      msh_col = msh_evaluate_col_cart(msh, iel, 'element_mask', boundary_mask);
      sp_col = sp_evaluate_col_vec(space, msh_col, 'value', false, 'gradient', true, 'element_mask', boundary_mask);
    end
