% EX_SURROGATE_POISSON_2D: Example which solves the Poisson problem in 2D
% using the surrogate method and compares it to the standard assembly.
%
% This examples considers the 2D benchmark with a constant sampling length
...
N = 160; % Number of knots in one dimension

% Construct manufactured solution
fprintf('Initializing problem...\n');
solution      = @(x,y) sin(20*pi*x).*sin(20*pi*y);
solution_dx   = @(x,y) 20*pi*cos(20*pi*x).*sin(20*pi*y);
solution_dy   = @(x,y) 20*pi*cos(20*pi*y).*sin(20*pi*x);
grad_solution = @(x, y) cat (1, ...
                  reshape(feval(solution_dx, x, y), [1, size(x)]), ...
                  reshape(feval(solution_dy, x, y), [1, size(x)]));
coeff = @(x,y) 1;
f = @(x,y) 800*pi^2*sin(20*pi*x).*sin(20*pi*y);

% Load surface
surface = GeomQuarterAnnulusWithBumps(p, N);

% Set up geometry and discrete spaces
geometry = geo_load(surface);
...
bc_solution = @(x, y, ind) solution(x,y);
...
tic;
K_surr = op_gradu_gradv_surrogate_2d(space, msh, coeff, M, q);
time_surr = toc;
