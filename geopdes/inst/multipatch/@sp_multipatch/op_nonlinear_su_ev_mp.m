% OP_NONLINEAR_SU_EV_MP: assemble the matrix A = 
%   mat = op_nonlinear_su_ev_mp (spu, msh, Stress, DStress, u_old, [patches]);
%
% INPUT:
%    
...
function A = op_nonlinear_su_ev_surrogate_2d_mp (spu, msh, Stress, DStress, u_old, patch_list)
  ...
  if ((spu.npatch ~= spu.npatch) || (spu.npatch ~= msh.npatch))
    error ('op_nonlinear_su_ev_mp: the number of patches does not coincide')
  end
