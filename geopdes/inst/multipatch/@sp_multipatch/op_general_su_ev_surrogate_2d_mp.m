% OP_GENERAL_SU_EV_SURROGATE_2D_MP: assemble the matrix A = [a(i,j)], a(i,j) = 1/2 (sigma (u_j), epsilon (v_i)), in a multipatch domain
% using the surrogate method.
%
%   mat = op_general_su_ev_surrogate_2d_mp (spu, msh, C, M, q, [patches]);
%
% INPUT:
%    
...
function A = op_general_su_ev_surrogate_2d_mp (spu, msh, C, surrogate_opts, patch_list)
  ...
  if ((spu.npatch ~= spu.npatch) || (spu.npatch ~= msh.npatch))
    error ('op_general_su_ev_surrogate_2d_mp: the number of patches does not coincide')
  end
  
  ncounter = 0;
  for iptc = patch_list
    [rs, cs, vs] = op_general_su_ev_surrogate_2d (spu.sp_patch{iptc}, msh.msh_patch{iptc}, C, surrogate_opts(iptc));
