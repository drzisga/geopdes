% SP_TO_VTK_MISES: Export von Mises Stress to VTK format for plotting.
%
%  sp_to_vtk (u, space, geometry, npts, filename, fieldname, CauchyStressEval)
%
% INPUT:
%     
...
function sp_to_vtk_mises_timedep (u, space, geometry, npts, startStep, timestep, filename, fieldname, CauchyStressEval )

  str1 = cat (2,'<?xml version="1.0"?> \n', ...
'<VTKFile type="Collection" version="0.1"> \n', ...
'<Collection> \n');

  str2 = cat (2, '<DataSet timestep="%d" part="%d" file="%s.vts"/> \n');
...
'</VTKFile> \n');
  ...
  fprintf (fid, str1);
  for step = startStep:timestep
    for iptc = 1:space.npatch
      filename_patch_without_path = cat (2, filename(ind:end), '_', num2str (step), '_', num2str (iptc));
      fprintf (fid, str2, step, iptc, filename_patch_without_path);
    end
  end
  fprintf (fid, str3);
  ...
  for iptc = 1:space.npatch
    filename_patch = cat (2, filename, '_', num2str (timestep), '_', num2str (iptc));
    
    if (isempty (space.dofs_ornt))
      sp_to_vtk_mises (u(space.gnum{iptc}), space.sp_patch{iptc}, geometry(iptc), npts, ...
                           filename_patch, fieldname, CauchyStressEval)
    else
      sp_to_vtk_mises (u(space.gnum{iptc}) .* space.dofs_ornt{iptc}', space.sp_patch{iptc}, geometry(iptc), npts, ...
                           filename_patch, fieldname, CauchyStressEval)
    end
  end
