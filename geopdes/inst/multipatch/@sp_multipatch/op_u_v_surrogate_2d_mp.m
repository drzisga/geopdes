% OP_U_V_SURROGATE_2D_MP: assemble the mass matrix M = [m(i,j)], m(i,j) = (mu u_j, v_i),
% in a multipatch domain using the surrogate method.
%
%   mat = op_u_v_surrogate_2d_mp (spu, msh, coeff, M, q, patch_list);
%
% INPUT:
%
...
function A = op_u_v_surrogate_2d_mp (spu, msh, coeff, surrogate_opts, patch_list)
  ...
  if ((spu.npatch ~= spu.npatch) || (spu.npatch ~= msh.npatch))
    error ('op_u_v_surrogate_2d_mp: the number of patches does not coincide')
  end
